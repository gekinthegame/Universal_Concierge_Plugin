...
mod tests {
    ...
    fn an_authorized_member_receives_a_ciphertext_only_manifest() {
        ...
        assert!(plan
            .decoded_node_kinds
            .iter()
            .any(|k| k == "encrypted_private"));
        assert!(
            plan.manifest.iter().any(|c| c.0 == ct_root),
            "the ciphertext root is present"
        );
        let plaintext_root = mem.private_conversions().unwrap()[0].plaintext_root.clone();
        assert!(
            !plan.manifest.iter().any(|c| c.0 == plaintext_root),
            "no plaintext source CID leaks"
        );
    ...
    fn a_member_without_a_sync_read_capability_is_refused() {
        ...
        assert!(
            mem.private_manifest_for_member(
                &ct_root,
                &write_only,
                &[],
                &descriptor,
    ...
    fn a_revoked_member_is_cut_off_from_the_manifest() {
        ...
        assert!(
            mem.private_manifest_for_member(&ct_root, &cap, &[], &descriptor, 2000, &revoked)
                .is_err(),
    ...
    fn the_pure_gate_requires_explicit_sync_read() {
        ...
        assert!(authorize_namespace_read(
            &cap,
            &[],
            &namespace,
            &descriptor,
